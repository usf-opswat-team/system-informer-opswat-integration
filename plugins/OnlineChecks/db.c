...
PPH_HASHTABLE MDCProcessObjectDb;
PH_QUEUED_LOCK ProcessObjectDbLock = PH_QUEUED_LOCK_INIT;
PH_QUEUED_LOCK MDCProcessObjectDbLock = PH_QUEUED_LOCK_INIT;
...
//Creates a hashtable for VirusTotal results
...
//Creates a hashtable for MetaDefender results
VOID InitializeMDCProcessDb(
    VOID
)
{
    MDCProcessObjectDb = PhCreateHashtable(
        sizeof(PPROCESS_DB_OBJECT),
        ProcessObjectDbEqualFunction,
        ProcessObjectDbHashFunction,
        64
...
//Delete the results hashtable for VirusTotal
...
//Delete the results hashtable for MetaDefender
VOID MDCCleanupProcessDb(
    VOID
)
{
    PhDereferenceObject(MDCProcessObjectDb);
}
...
ULONG GetNumberOfMDCProcessDbObjects(
    VOID
)
{
    return MDCProcessObjectDb->Count;
}
...
VOID LockMDCProcessDb(
    VOID
)
{
    PhAcquireQueuedLockExclusive(&MDCProcessObjectDbLock);
}
...
VOID UnlockMDCProcessDb(
    VOID
)
{
    PhReleaseQueuedLockExclusive(&MDCProcessObjectDbLock);
}
...
PPROCESS_DB_OBJECT FindMDCProcessDbObject(
    _In_ PPH_STRINGREF FileName
)
{
    PROCESS_DB_OBJECT lookupObject;
    ...
    objectPtr = PhFindEntryHashtable(MDCProcessObjectDb, &lookupObjectPtr);
...
PPROCESS_DB_OBJECT CreateMDCProcessDbObject(
    _In_ PPH_STRING FileName,
    _In_opt_ INT64 Positives,
    _In_opt_ INT64 Total,
    _In_opt_ PPH_STRING Hash
)
{
    ...
    realObject = PhAddEntryHashtableEx(MDCProcessObjectDb, &object, &added);
...
PPROCESS_DB_OBJECT UpdateMDCProcessDbObject(
    _In_ PPH_STRING FileName,
    _In_opt_ INT64 Positives,
    _In_opt_ INT64 Total,
    _In_opt_ PPH_STRING Hash
)
{
    ...
    object = FindMDCProcessDbObject(&FileName->sr);
