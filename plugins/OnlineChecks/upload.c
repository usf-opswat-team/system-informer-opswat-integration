...
SERVICE_INFO UploadServiceInfo[] =
{
    ...
    { MENUITEM_METADEFENDER_UPLOAD, L"api.metadefender.com", L"/v4/file", L"file" },
    { MENUITEM_METADEFENDER_UPLOAD_SERVICE, L"api.metadefender.com", L"/v4/file", L"file" },
    { MENUITEM_METADEFENDER_UPLOAD_WEB, L"api.metadefender.com", L"/v4/file", L"file" },
    { MENUITEM_METADEFENDER_UPLOAD_WEB_SERVICE, L"api.metadefender.com", L"/v4/file", L"file" },//Used by upload file thread start
};
    ...
    )
{
    ...
    if (
        Context->Service == MENUITEM_METADEFENDER_UPLOAD ||
        Context->Service == MENUITEM_METADEFENDER_UPLOAD_SERVICE ||
        Context->Service == MENUITEM_METADEFENDER_UPLOAD_WEB ||
        Context->Service == MENUITEM_METADEFENDER_UPLOAD_WEB_SERVICE
        )
    {
        PPH_BYTES serviceHash;
        PPH_STRING httpHeader;
        ...
        serviceHash = MetaDefenderGetApiKey(Context->DialogHandle);
...
//This is our upload routine if no hash is found we will upload the file
    ...
    )
{
        ...
        )
    {
        ...
        case MENUITEM_METADEFENDER_UPLOAD:
        case MENUITEM_METADEFENDER_UPLOAD_SERVICE:
        case MENUITEM_METADEFENDER_UPLOAD_WEB:
        case MENUITEM_METADEFENDER_UPLOAD_WEB_SERVICE:
            {

                PPH_BYTES jsonString; //variable to hold response of POST
                PVOID jsonRootObject;
                PPH_STRING status_message;

                if (!(jsonString = PhHttpSocketDownloadString(httpContext, FALSE))) //If unable to get response of POST 
                {
                    RaiseUploadError(context, L"Unable to download the response.", GetLastError());
                    goto CleanupExit;
                }

                if (jsonRootObject = PhCreateJsonParser(jsonString->Buffer)) //getting json data from json? 
                {
                    status_message = PhGetJsonValueAsString(jsonRootObject, "status"); //we can get what "status" is but what are the options for status
                    
                    if(PhEqualStringZ(PhGetString(status_message), L"inqueue", 0))//if status is complete or good?
                    {
                        PPH_STRING jsonDataIDString; //variable to hold "data_id" which is a response from the POST

                        jsonDataIDString = PhGetJsonValueAsString(jsonRootObject, "data_id"); //parsing json to get data_id and store in jsonDataIDString

                        if (!PhIsNullOrEmptyString(jsonDataIDString)) //if jsonDataIDstring != NULL then this evaluates to TRUE
                        {
                            ////////////////// perform GET request and parse JSON data start /////////////////////////////////////
                            PVOID rootJsonObject; //to be used in the performSubRequest function /////////////////(Note: very similair name to jsonRootObject!!!)////////////
                            INT64 progress_percentage;
                            subObjectName = PhConcatStrings2(L"/v4/file/", PhGetString(jsonDataIDString));
                            ...
                            do
                            {
                                ...
                                {
                                    goto CleanupExit; // if the GET does not work GO TO CLEAN UP
                                }

                                if (rootJsonObject = PhCreateJsonParser(subRequestBuffer->Buffer)) //this rootJsonObject will now contain the response of GET Analyze File
                                {

                                    PVOID scanResultsJsonObject = PhGetJsonObject(rootJsonObject, "scan_results"); //getting scan_results Json object

                                    progress_percentage = PhGetJsonValueAsInt64(scanResultsJsonObject, "progress_percentage"); //get whatever you need from JSON object

                                    if (progress_percentage == 100) //if the GET is not ready....in the future we will loop for a bit POLLING the GET
                                    {
                                        //variables that we will add to the column
                                        INT64 total_detected_avs = PhGetJsonValueAsInt64(scanResultsJsonObject, "total_detected_avs");
                                        INT64 total_avs = PhGetJsonValueAsInt64(scanResultsJsonObject, "total_avs");

                                        UpdateMDCProcessDbObject(
                                            context->FileName,
                                            total_detected_avs,
                                            total_avs,
                                            context->FileHash
                                        ...
                                        {
                                            PhMoveReference(&context->LaunchCommand, PhFormatString(
                                                L"https://metadefender.opswat.com/results/file/%s/hash/overview",
                                ...
                                ////////////////// perform GET request and parse JSON data end  ////////////////////////////////////
                                PhDelayExecution(1 * 500); //making a delay of half a second so we don't burn API limit
                            } while (progress_percentage != 100);

                            PhFreeJsonObject(rootJsonObject);       //then we will clear and free these
                            
                        }
                        if (jsonDataIDString) //destory object to avoid mem leak
                            PhDereferenceObject(jsonDataIDString);
                    }
                    else
                    {
                        RaiseUploadError(context, L"MetaDefender Cloud API error.", (ULONG)-1); //need to change error code to "status"
                        PhDereferenceObject(jsonString);
                        goto CleanupExit;
                    }
    ...
    )
{
    ...
    if (NT_SUCCESS(status = PhGetFileSize(fileHandle, &fileSize64)))
    {
        ...
        {
            if (fileSize64.QuadPart > 140 * 1024 * 1024) // 140 MB
            {
                RaiseUploadError(context, L"The file is too large (over 140 MB)", ERROR_FILE_TOO_LARGE);
                goto CleanupExit;
            }
        }
        else
    ...
    case MENUITEM_METADEFENDER_UPLOAD_WEB_SERVICE:
        {
            ...
            PSTR quote = NULL;
            PVOID rootJsonObject; 
            PVOID errorJsonObject;
            PVOID results;
            INT daysSince = 0;
            INT count = 0;
            INT64 total_detected_avs;
            INT64 total_avs;
            ...
            if (!NT_SUCCESS(status = HashFileAndResetPosition(fileHandle, &fileSize64, Sha256HashAlgorithm, &tempHashString))) //calling hashing function
            {
                RaiseUploadError(context, L"Unable to hash the file", PhNtStatusToDosError(status)); //if unsucessful 
                goto CleanupExit;
            }

            context->FileHash = tempHashString; //store hash from hashing function in context struct
            subObjectName = PhConcatStrings2(L"/v4/hash/", PhGetString(context->FileHash)); //adding GET API handle

            if (!(subRequestBuffer = PerformSubRequest( //performing GET API call
                context,
                serviceInfo->HostName,
                subObjectName->Buffer
            )))
            ...
            if (rootJsonObject = PhCreateJsonParser(subRequestBuffer->Buffer)) //if we can parse the rootJsonObject from the GET API call
            {
                if (errorJsonObject = PhGetJsonObject(rootJsonObject, "error")) //if error is in rootJsonObject handle with if else statments
                {
                    INT64 code = PhGetJsonValueAsInt64(errorJsonObject, "code");
                    if (code == 404003)
                    {
                        PostMessage(context->DialogHandle, UM_UPLOAD, 0, 0);
                    }
                    ...
                    {
                        RaiseUploadError(context, L"Unsucessfull hash", (ULONG)code);

                    }
                    //Check if API limit reached error code 429000
                    else if (code == 429000)
                    {
                        //Show a custom error message and provide a link to upgrade usage limits
                        DialogBox(PluginInstance->DllBase, MAKEINTRESOURCE(IDD_MDC_LIMIT), context->DialogHandle, LimitDlgProc);
                        //Dismiss the "Scanning..." task dialog box
                        SendMessage(context->DialogHandle, TDM_CLICK_BUTTON, IDOK, 0);
                    }
                    else
                    {
                        RaiseUploadError(context, L"MetaDefender Cloud API Unknown Error.", (ULONG)code);

                    }
                    PhFreeJsonObject(errorJsonObject);
                }
                else { //else error is not in the rootJsonObject so we did not get an error...SUCCESS
                    if (results = PhGetJsonObject(rootJsonObject, "scan_results")) //if we can parse a results from rootJsonObject SUCCESSFUL PARSE
                    {
                        total_detected_avs = PhGetJsonValueAsInt64(results, "total_detected_avs");
                        total_avs = PhGetJsonValueAsInt64(results, "total_avs");
                        context->Detected = PhFormatString(L"%lld", total_detected_avs);
                        context->MaxDetected = PhFormatString(L"%lld", total_avs);
                        PhFreeJsonObject(results);
                    }
                    if (results = PhGetJsonObject(rootJsonObject, "file_info")) //if we can parse a results from rootJsonObject SUCCESSFUL PARSE
                    {
                        context->BaseFileName = PhGetJsonValueAsString(results, "display_name");
                        context->LastAnalysisDate = PhGetJsonValueAsString(results, "upload_timestamp");
                        if (!PhIsNullOrEmptyString(context->LastAnalysisDate))
                        {
                            PhMoveReference(&context->LastAnalysisDate, MetaDefenderStringToTime(context->LastAnalysisDate));
                            context->LastAnalysisAgo = MetaDefenderNumDaysSince(PhGetJsonValueAsString(results, "upload_timestamp"));
                            count = swscanf(
                                PhGetString(context->LastAnalysisAgo), L"%d",
                                &daysSince
                            );
                        }
                        context->FileSize = PhFormatSize(PhGetJsonValueAsInt64(results, "file_size"), ULONG_MAX);
                        PhFreeJsonObject(results);
                        
                    }

                    CreateMDCProcessDbObject( //creating column object from GET API call
                        context->FileName,
                        total_detected_avs,
                        total_avs,
                        context->FileHash
                    ...
                    else if (PhIsNullOrEmptyString(context->LastAnalysisDate) || daysSince > 3)
                    {
                        PostMessage(context->DialogHandle, UM_EXISTS_META, 0, 0); //closing diablog box for scanning progress after successful hash
                    }
...
INT_PTR CALLBACK LimitDlgProc(_In_ HWND hDlg, _In_ UINT message,
        ...
        case NM_RETURN:
            {
                ...
                {
                    PhShellExecute(NULL, item.szUrl, NULL);
                }
