...
BOOLEAN MetaDefenderScanningEnabled = FALSE;
...
VOID ProcessesUpdatedCallback( //changed for multiscanning
    ...
    )
{
    ...
    //if (!VirusTotalScanningEnabled && !MetaDefenderScanningEnabled)
    //    return;
    ...
    while (listEntry != &ProcessListHead)
    {
        ...
        {
            if (VirusTotalScanningEnabled && !extension->ResultValid) //Added check for VirusTotalScanningEnabled
            {
            ...
            //if (MetaDefenderScanningEnabled && !extension->MDCResultValid) //Added check for MetaDefenderScanningEnabled
            if (!extension->MDCResultValid) //Always run so single scans can populate columns
            {
                PPROCESS_DB_OBJECT object;

                //Add other if statement to get DbObject from MDC db
                //PhSwapReference(&extension->MetaDefenderResult, object->Result)
                if (filePath && (object = FindMDCProcessDbObject(&filePath->sr)))
                {
                    //extension->Stage1 = TRUE;
                    //extension->MDCResultValid = TRUE; //Keep FALSE to allow column updating for new rescans
            ...
            if (VirusTotalScanningEnabled && !extension->Stage1) //Added check for MetaDefenderScanningEnabled
            {
            ...
            if (MetaDefenderScanningEnabled && !extension->MDCStage1) {
                if (filePath && !MDCGetCachedResult(filePath))
                {
                    MDCAddCacheResult(filePath, extension);
                }

                extension->MDCStage1 = TRUE;
            }
        }
...
//Effectively this is a run-once-on-startup function
VOID NTAPI LoadCallback(
    _In_opt_ PVOID Parameter,//Most callbacks give you additional information. This event doesn't give you any.
    _In_opt_ PVOID Context//This is the arbitrary PVOID value you specified in PhRegisterCallback
    )
{   //DO stuff...
    ...
    //Always initialize the MetaDefender Cloud results database so it can be used with SendTo menu scans
    InitializeMDCProcessDb();

    //Check if MetaDefender setting and set setting flag, do initialization steps if enabled
    if (MetaDefenderScanningEnabled = !!PhGetIntegerSetting(SETTING_NAME_METADEFENDER_SCAN_ENABLED))
    {
        //InitializeMDCProcessDb();
        InitializeMDCProcessMonitor();
    }
}
...
VOID NTAPI MenuItemCallback(
    ...
    //_In_opt == parameters are optional 
    _In_opt_ PVOID Parameter,
    _In_opt_ PVOID Context
    )
{
    ...
    if (!menuItem)
        return;
    //switch case to check which menu item was selected 
    switch (menuItem->Id)
    {
    case ENABLE_SERVICE_VIRUSTOTAL:
        {   //ULONG == unsigned long == integer from 0 to 4,294,967,295
            ULONG scanningEnabled = !VirusTotalScanningEnabled; //VirusTotalScanningEnabled is a boolean
        ...
        break;
    case ENABLE_SERVICE_METADEFENDER:
        {
            ULONG scanningEnabled = !MetaDefenderScanningEnabled;

            PhSetIntegerSetting(SETTING_NAME_METADEFENDER_SCAN_ENABLED, scanningEnabled);

            if (MetaDefenderScanningEnabled != scanningEnabled)
            {
                INT result = IDOK;
                TASKDIALOGCONFIG config;
                ...
                config.hMainIcon = PhGetApplicationIcon(FALSE);
                config.cxWidth = 180;
                config.pszWindowTitle = L"System Informer - MetaDefender Cloud";
                config.pszMainInstruction = L"MetaDefender scanning requires a restart of System Informer.";
                config.pszContent = L"Do you want to restart System Informer now?";
    ...
    case MENUITEM_METADEFENDER_UPLOAD:
        UploadToOnlineService(menuItem->Context, MENUITEM_METADEFENDER_UPLOAD);
        break;
    case MENUITEM_METADEFENDER_UPLOAD_SERVICE:
        UploadServiceToOnlineService(menuItem->Context, MENUITEM_METADEFENDER_UPLOAD);
        break;
    case MENUITEM_METADEFENDER_UPLOAD_WEB:
        UploadToOnlineService(menuItem->Context, MENUITEM_METADEFENDER_UPLOAD_WEB);
        break;
    case MENUITEM_METADEFENDER_UPLOAD_WEB_SERVICE:
        UploadServiceToOnlineService(menuItem->Context, MENUITEM_METADEFENDER_UPLOAD_WEB);
        break;
...
//function for Tools -> Online Checks -> enable scanning, upload file to/for...
    ...
    )
{
    ...
    PPH_EMENU_ITEM enableMenuItemMDC; //Return for creating MetaDefender menu item
    ...
    //online checks -> Enable VirusTotal scanning, Upload file ... etc
    ...
    //Needed to add defintion in onlnchk.h -> #define ENABLE_SERVICE_METADEFENDER 130
    PhInsertEMenuItem(onlineMenuItem, enableMenuItemMDC = PhPluginCreateEMenuItem(PluginInstance, 0, ENABLE_SERVICE_METADEFENDER, L"&Enable MetaDefender Cloud scanning", NULL), ULONG_MAX);
    PhInsertEMenuItem(onlineMenuItem, PhCreateEMenuSeparator(), ULONG_MAX); //this represents the a line ex: ----------------------
    ...
    //Place checkmark next to enabled menu options
    if (VirusTotalScanningEnabled)
        enableMenuItem->Flags |= PH_EMENU_CHECKED;

    if (MetaDefenderScanningEnabled)
        enableMenuItemMDC->Flags |= PH_EMENU_CHECKED;
}
    ...
    )
{
    ...
    PhInsertEMenuItem(sendToMenu, PhPluginCreateEMenuItem(PluginInstance, 0, MENUITEM_METADEFENDER_UPLOAD, L"&MetaDefender", FileName), ULONG_MAX);
    PhInsertEMenuItem(sendToMenu, PhPluginCreateEMenuItem(PluginInstance, 0, MENUITEM_METADEFENDER_UPLOAD_WEB, L"View MetaDefender &Report", FileName), ULONG_MAX);
    ...
    )
{
    ...
    PhInsertEMenuItem(sendToMenu, PhPluginCreateEMenuItem(PluginInstance, 0, MENUITEM_METADEFENDER_UPLOAD_SERVICE, L"Metadefender API", serviceItem ? serviceItem : NULL), ULONG_MAX);
    PhInsertEMenuItem(sendToMenu, PhPluginCreateEMenuItem(PluginInstance, 0, MENUITEM_METADEFENDER_UPLOAD_WEB_SERVICE, L"Metadefender Website", serviceItem ? serviceItem : NULL), ULONG_MAX);
...
//This function defines the VirusTotal column in the process tab
    ...
    )
{
    ...
    //Create MetaDefender Cloud column
    memset(&column2, 0, sizeof(PH_TREENEW_COLUMN));
    column2.Text = L"MetaDefender";
    column2.Width = 140;
    column2.Alignment = PH_ALIGN_CENTER;
    column2.CustomDraw = TRUE;
    column2.Context = info->TreeNewHandle; // Context

    PhPluginAddTreeNewColumn(PluginInstance, info->CmData, &column2, COLUMN_ID_METADEFENDER_PROCESS, NULL, VirusTotalProcessNodeSortFunction);
}
...
//This function defines the VirusTotal column in the services tab
    ...
    case TreeNewCustomDraw:
        {
            ...
            if (!VirusTotalScanningEnabled && message->SubId < 4)
            {
            ...
            //if (!MetaDefenderScanningEnabled && message->SubId >= 4)
            //{
            //    static PH_STRINGREF disabledText = PH_STRINGREF_INIT(L"MDC disabled");
            ...
            //Get results for column by case of column ID
            switch (message->SubId)
            {
            case COLUMN_ID_VIUSTOTAL_PROCESS:
                {
            ...
            case COLUMN_ID_VIUSTOTAL_SERVICE:
                ...
                break;
            //Note: PhPluginGetObjectExtension doesn't seem to care if we change the object
            //      So adding MetaDenfender result to the PPROCESS_EXTENSION struct is fine
            case COLUMN_ID_METADEFENDER_PROCESS:
                {
                    PPH_PROCESS_NODE processNode = (PPH_PROCESS_NODE)customDraw->Node;
                ...
                break;
            case COLUMN_ID_METADEFENDER_MODULE:
                {
                    PPH_MODULE_NODE moduleNode = (PPH_MODULE_NODE)customDraw->Node;
                ...
                break;
            case COLUMN_ID_METADEFENDER_SERVICE:
                {
                    PPH_SERVICE_NODE serviceNode = (PPH_SERVICE_NODE)customDraw->Node;
            ...
            case COLUMN_ID_METADEFENDER_SERVICE:
                {
                    text = PhGetStringRef(extension->MetaDefenderResult);
                }
                break;
            }
...
LOGICAL DllMain(
    _In_ HINSTANCE Instance, //Base address of plugin DLL. PH needs to know this value, which is why you pass it to PhRegisterPlugin.
    _In_ ULONG Reason,
    ...
    case DLL_PROCESS_ATTACH:
        {
            ...
            PH_SETTING_CREATE settings[] =
            {
                ...
                //Add the MetaDefender setting
                { IntegerSettingType, SETTING_NAME_METADEFENDER_SCAN_ENABLED, L"0" },
                { StringSettingType, SETTING_NAME_METADEFENDER_API_KEY, L"" }
            };
            ...
            //Process Hacker notifies your plugin of events. You must use PhRegisterCallback to setup handlers for these events. 
            PhRegisterCallback(
                //PhGetPluginCallback gets a pointer to a PPH_CALLBACK object that describers the event you're interested in
                PhGetPluginCallback(PluginInstance, PluginCallbackLoad),
                //The address of your handler function
                LoadCallback,
                //Arbitrary PVOID value passed to your handler function (in this case, LoadCallback) in the context parameter
                //Most of the time you won't need this
                NULL,
                //A pointer too a PH_CAALLBACK_REGISTRATION structure. This structure must be avaliable as long
                //as you handler function is registered (i.e. until you call PHUnregisterCallback).
                //For best results, declare this as a global variable (see the top of this code sample)
