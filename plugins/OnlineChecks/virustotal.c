    ...
    )
{
    ...
    /*
    for (i = 0; i < VirusTotalList->Count; i++) {
        PMDC_FILE_HASH_ENTRY extension = VirusTotalList->Items[i];
        if (extension->FileHash == NULL) continue;
        OutputDebugString(L"Hash: ");
        OutputDebugString(extension->FileHash->Buffer);
        OutputDebugString(L"\n");
        OutputDebugString(L"File Name: ");
        OutputDebugString(extension->FileName->Buffer);
        OutputDebugString(L"\n");
    }*/
    ...
    )
{
    ...
    do
    {
        ...
        PhAcquireQueuedLockExclusive(&ProcessListLock); //aquire lock to add process to list

        //Where does VirusTotalList get initialized????
        //I think VirustTotalList holds the # of processes?
        for (i = 0; i < VirusTotalList->Count; i++) //VirusTotalList is a PPH_LIST which holds ULONG count
        {
            PVIRUSTOTAL_FILE_HASH_ENTRY extension = VirusTotalList->Items[i]; //Items is a PVOID*
        ...
        PhReleaseQueuedLockExclusive(&ProcessListLock); //release lock, process added to list
        ...
        {
            VirusTotalBuildJsonArray(resultTempList->Items[i], jsonArray); //get the process and make a JsonArray
        }

        ////////////////////// Try to get rootJsonObject ////////////////////////////////////////
        ...
        if (virusTotalResults = VirusTotalJsonToResultList(dataJsonObject)) //JSON to virusTotalResults
        {
            for (i = 0; i < virusTotalResults->Count; i++)
            {
                ...
                if (result->Found)
                {
                    PVIRUSTOTAL_FILE_HASH_ENTRY entry = VirusTotalGetCachedResultFromHash(result->FileHash); //get fileHash
                    ...
                    if (entry && !entry->Processed) //if entry is not null and not processed
                    {
                        ...
                        if (!FindProcessDbObject(&entry->FileName->sr))
                        {
                            ...
                            CreateProcessDbObject( //call that thing christopher was working on
                                entry->FileName,
                                entry->Positives,
                                entry->Total,
                                result->FileHash
    ...
    )
{
    ...
    PhCreateThreadEx(&VirusTotalHandle, VirusTotalProcessApiThread, NULL); //ApiThread is our work horse for scanning
}
